...
if st.sidebar.button("▶️ Run Simulation", type="primary"):
    ...
    with tab1:
        ...
        with st.spinner("Rendering Video Player..."):
            # to_jshtml() creates a player with Play/Pause/Loop/Grid controls
            components = st.components.v1.html(ani.to_jshtml(), height=700, scrolling=False)

    # --- TAB 2: METRICS (MATCHING REQUEST) ---
    with tab2:
        ...
        axs[0].plot(t, np.degrees(history['c_theta']), label='Sphere Spin', color='orange')
