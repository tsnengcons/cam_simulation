...
# Use r""" for raw strings to handle LaTeX backslashes correctly
st.markdown(r"""
...
with st.sidebar.expander("C. Numerical Solver Settings", expanded=False):
    ...
    # Raw string added for LaTeX delta
    dt_selection = st.selectbox(r"Time Step $\Delta t$", list(dt_map.keys()), index=0)
...
if st.session_state.ani_html:
    ...
    st.info("💡 **Interaction:** Use the slider below the graph to scrub through time. The shiny red dots on the right indicate the instantaneous state of the system.")
